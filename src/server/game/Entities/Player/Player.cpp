...
bool Player::Create(uint32 guidlow, const std::string& name, uint8 race, uint8 class_, uint8 gender, uint8 skin, uint8 face, uint8 hairStyle, uint8 hairColor, uint8 facialHair, uint8 /*outfitId*/)
{
    ...
    if (GetSession()->GetSecurity() >= SEC_TRIAL_GM)
    {
...
void Player::Update(uint32 p_time)
{
    ...
    // handle ress about 4 seconds
    if (isDead() && time_before_ressed <= p_time)
    {
        ResurrectPlayer(GetSession()->GetSecurity() > SEC_PLAYER ? 1.0f : 0.5f);
        SpawnCorpseBones();
        SaveToDB();
...
bool Player::TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options)
{
    ...
    if ((GetSession()->GetSecurity() < SEC_GM ) && sDisableMgr->IsDisabledFor(DISABLE_TYPE_MAP, mapid, this))
    {
...
void Player::InitTalentForLevel()
{
    ...
    else
    {
        ...
        {
            if (GetSession()->GetSecurity() < SEC_OWNER)
                resetTalents(true);
            else
                SetFreeTalentPoints(0);
        }
...
{	
	...
	else if (hasInstantHasteCap && m_baseRatingValue[CR_HASTE_MELEE] < sWorld->getIntConfig(CONFIG_PLAYER_HASTE_CAP))
	{
        ...
        ApplyCastTimePercentMod(m_baseRatingValue[CR_HASTE_SPELL] / GetRatingCoefficient(CR_HASTE_SPELL), true);

		if (GetShapeshiftForm())
		{
			SpellShapeshiftEntry const* ssEntry = sSpellShapeshiftStore.LookupEntry(GetShapeshiftForm());
			if (ssEntry && ssEntry->Id)
			{
				SetAttackTime(BASE_ATTACK, ssEntry->Id);
				SetAttackTime(OFF_ATTACK, ssEntry->Id);
