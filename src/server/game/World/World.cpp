...
void World::LoadConfigSettings(bool reload)
{
    ...
    m_int_configs[CONFIG_GM_LEVEL_IN_GM_LIST]   = sConfig->GetIntDefault("GM.InGMList.Level", SEC_OWNER);
    m_int_configs[CONFIG_GM_LEVEL_IN_WHO_LIST]  = sConfig->GetIntDefault("GM.InWhoList.Level", SEC_OWNER);
    ...
    m_int_configs[CONFIG_INSTANT_LOGOUT] = sConfig->GetIntDefault("InstantLogout", SEC_TRIAL_GM);
	...
	// Player haste cap
	m_int_configs[CONFIG_PLAYER_HASTE_CAP] = sConfig->GetIntDefault("Player.HasteCap", 0);
...
void World::SendGMText(int32 string_id, ...)
{
    ...
    for (SessionMap::iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        ...
        if (itr->second->GetSecurity() < SEC_TRIAL_GM)
            continue;
