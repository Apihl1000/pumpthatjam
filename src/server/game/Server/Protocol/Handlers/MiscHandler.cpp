...
void WorldSession::HandleAddFriendOpcodeCallBack(QueryResult result, std::string friendNote)
{
    ...
    if (result)
    {
        ...
        if (GetSecurity() >= SEC_TRIAL_GM || sWorld->getBoolConfig(CONFIG_ALLOW_GM_FRIEND) || sAccountMgr->GetSecurity(friendAcctid, realmID) < SEC_TRIAL_GM)
        {
            if (friendGuid)
            {
                if (friendGuid == GetPlayer()->GetGUID())
                    friendResult = FRIEND_SELF;
                else if (GetPlayer()->GetTeam() != team && !sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_ADD_FRIEND) && GetSecurity() < SEC_GM )
