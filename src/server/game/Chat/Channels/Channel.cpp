...
void Channel::Join(uint64 p, const char *pass)
{
    ...
    if (m_announce && (!plr || plr->GetSession()->GetSecurity() < SEC_GM  || !sWorld->getBoolConfig(CONFIG_SILENTLY_GM_JOIN_TO_CHANNEL)))
    {
        MakeJoined(&data, p);
        SendToAll(&data);
    }
...
void Channel::KickOrBan(uint64 good, const char *badname, bool ban)
{
    ...
    else if (!players[good].IsModerator() && sec < SEC_GM )
    {
        WorldPacket data;
    ...
    else
    {
        ...
        else if (sec < SEC_GM  && bad->GetGUID() == m_ownerGUID && good != m_ownerGUID)
        {
            WorldPacket data;
            MakeNotOwner(&data);
            SendToOne(&data, good);
